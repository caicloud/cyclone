...
func ReplaceFile(destPath string, content io.Reader) error {
	...
	if err != nil {
		log.Errorf("Unable to remove old file %s as %v", destPath, err)
		return err
	}

	file, err := os.OpenFile(destPath, os.O_CREATE|os.O_RDWR, 0755)
	if err != nil {
		log.Errorf("Unable to create new file %s as %v", destPath, err)
		return err
	}
	_, err = io.Copy(file, content)
	if err != nil {
		log.Errorf("Unable to write content to file %s as %v", destPath, err)
		return err
	}
	return file.Close()
}
