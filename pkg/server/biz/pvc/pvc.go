...
func CreatePVC(tenantName, storageClass, size string, namespace string, client *kubernetes.Clientset, inCtrVPC bool) error {
	// parse quantity
...
func UpdatePVC(tenantName, storageClass, size string, namespace string, client *kubernetes.Clientset, inCtrVPC bool) error {
	...
	// Create a new PVC
	err = retry.OnError(backoff, func() error {
		return CreatePVC(tenantName, storageClass, size, namespace, client, inCtrVPC)
	})
	if err != nil {
		return err
	}
