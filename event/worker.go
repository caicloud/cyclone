...
import (
	...
	k8s_client "github.com/caicloud/cyclone/kubernetes"
...
const (
	...
	WORKER_KUBERNETES_HOST      = "WORKER_KUBERNETES_HOST"
	WORKER_KUBERNETES_TOKEN     = "WORKER_KUBERNETES_TOKEN"
	WORKER_KUBERNETES_NAMESPACE = "WORKER_KUBERNETES_NAMESPACE"
	...
	WORKER_PROVIDER_DOCKER = "docker"
	WORKER_PROVIDER_K8S    = "kubernetes"
)
...
var (
	...
	// Worker provider, docker or kubernetes.
	WorkerProvider = WORKER_PROVIDER_DOCKER
)
...
func NewWorker(event *api.Event) (*Worker, error) {
	...
	if WorkerProvider == WORKER_PROVIDER_K8S {
		host := osutil.GetStringEnv(WORKER_KUBERNETES_HOST, "")
		token := osutil.GetStringEnv(WORKER_KUBERNETES_TOKEN, "")
		namespace := osutil.GetStringEnv(WORKER_KUBERNETES_NAMESPACE, "default")

		w := &Worker{
			provider: WORKER_PROVIDER_K8S,
			k8sCompose: K8sCompose{
				host:      host,
				token:     token,
				namespace: namespace,
		...
		return w, nil
	} else if WorkerProvider == WORKER_PROVIDER_DOCKER {
		...
		w := &Worker{
			...
			provider:    WORKER_PROVIDER_DOCKER,
		}
...
func (w *Worker) DoWork(event *api.Event) (err error) {
	if w.provider == WORKER_PROVIDER_K8S {
...
func (w *Worker) Fire() error {
	...
	} else if w.provider == WORKER_PROVIDER_DOCKER {
...
func toCreatePodConfig(event *api.Event, cpu, memory int64) *k8s_api.Pod {
	...
	workerContainer := k8s_api.Container{
		...
		Env: []k8s_api.EnvVar{envEventID, envServerHost, envregistryLocation, envregistryUsername,
			envregistryPassword, envconsoleWebEndpoint, envclairServerIP, envgitlabServer, envLogServer},
